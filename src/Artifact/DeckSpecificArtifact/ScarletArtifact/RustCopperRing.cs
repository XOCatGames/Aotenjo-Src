...
namespace Aotenjo
{
    public class RustCopperRingArtifact : Artifact
    {
        private const int AMOUNT = 3; //锁定level

        public RustCopperRingArtifact() : base("rust_copper_ring", Rarity.RARE)
        {
        ...
        private void OnGetCoreLevel(PlayerGetScarletCoreLevelEvent evt)
        {
            ...
            if (scarlet.IsCompatibleWithUnusedCategory(evt.cat))
            {
                evt.level = AMOUNT;
            }
        }
