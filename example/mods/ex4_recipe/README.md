...
退出游戏，把本文件夹直接放到 `Aotenjo_Data/StreamingAssets/mods/`，使下一层就是 `modinfo.json`，完整重启。依次 `give tutorial_recipe:copper_token` 与 `give tutorial_recipe:silver_token`，应消耗输入并获得双代币，计分加5番。无需安装其他示例。
