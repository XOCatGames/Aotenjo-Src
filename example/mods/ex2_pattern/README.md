...
先退出游戏，把本文件夹放进 `Aotenjo_Data/StreamingAssets/mods/`。放好后，应该能直接找到 `mods/ex2_pattern/modinfo.json`，然后重新启动游戏。

选择标准范围的套组开一个新局，输入 `upgradeYaku custom_yaku:tutorial_yaku:all_twos 1`，给全二增加1级。先打出全部由二组成的组合，再试试混入其他牌，应该只有前者成立。示例还把全二加入了火包，对应索引为2。
